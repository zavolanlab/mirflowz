...
```bash
bash test/test_integration_workflow_with_singularity/test_local.sh
```

- Test workflow on local machine with **Conda**:

```bash
bash test/test_integration_workflow_with_conda/test_local.sh
```
...
```bash
bash test/test_integration_workflow_with_singularity/test_slurm.sh
```
...
```bash
bash test/test_integration_workflow_with_conda/test_slurm.sh
```
...
```bash
bash test/test_rule_graph/rule_graph.sh
```
...
to remove all artifacts generated by the test runs on their respective
directories:

```bash
bash test/test_path/test_cleanup.sh
```
